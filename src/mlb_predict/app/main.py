...
async def page_today_odds(request: Request):
    """Today's sportsbook lines (Odds API) in a dedicated view."""
    if not is_ready():
        return _init_page(request)
    return templates.TemplateResponse(request, "today_odds.html", _ctx(request))
