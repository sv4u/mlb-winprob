...
def run_expanding_cv(
    ...
    *,
    ...
    # The pre-computed dict is then looked up in the CV loop (no re-training).
    stage1_data: dict[str, Any] | None = None
    stage1_precomputed: dict[int, pd.DataFrame] = {}
    if enable_stage1:
        try:
    ...
    for i, eval_season in enumerate(seasons):
        ...
        # --- Stage 1: look up pre-computed features --------------------------
        if stage1_precomputed:
            for s in train_seasons + [eval_season]:
                if s in stage1_precomputed:
                    season_dfs[s] = stage1_precomputed[s]
