...
- **Web dashboard** — a full interactive web UI covering games, standings, leaders, players, odds, and sabermetric team/player statistics (traditional + advanced).
...
| Sabermetrics dashboard (traditional + advanced team/player stats) | Play-by-play derived metrics (launch angle, sprint speed) as first-class UI |
...
## 1.4 Current season default (UI + API)

All pages and API endpoints that accept an optional `season` query parameter use a **single MLB-aware default** when the parameter is omitted:

| Calendar month | Default season year |
|---|---|
| January | Previous calendar year (offseason before spring training) |
| February–October | Current calendar year (spring training + regular season) |
| November–December | Current calendar year (recently completed season still labeled by start year) |

Implementation: `src/mlb_predict/season.py` (`current_mlb_season`, `resolve_season`). Exposed to clients via `GET /api/current-season`. JavaScript mirror: `currentMlbSeason()` and `fetchCurrentSeason()` in `app/static/base.js`.

**Cold start / hot start:** On application startup (including model hot-swap reload), the UI must select this default season—not the latest ingested feature season and not a hard-coded year. If the default season has no rows in the feature matrix yet (e.g. 2026 before ingest), the UI still selects it and shows an empty state with a link to **Admin → Update season**.
...
- `team_stats.py` — multi-window rolling (7/14/15/30/60 games), EWMA (span=20), home/away splits (20-game venue sub-windows), streaks, rest, run distribution (30-game std / one-run win%)
...
- `main.py` — FastAPI application: game browser, 2026 season page, game detail, upsets, CV summary; SHAP attribution; 4-way auto-bootstrap; **deferred model load** (`asyncio.to_thread(try_startup)`) so HTTP accepts connections while the stacked model and Stage 1 injection finish; **combined lifespan** runs the MCP app’s lifespan after the core app lifespan so `/mcp` tools see loaded data
- `data_cache.py` — loads features via DuckDB store (fast path) or Parquet files (fallback); normalizes date types; pre-computes probabilities for all games; resolves **git commit** from `GIT_COMMIT`, `GIT_COMMIT` file, or `git rev-parse`; optional **`CHANGELOG.txt`** (pipe-delimited lines) for the wiki / version UI
- `admin.py` — async pipeline runner (ingest / update / retrain), step-level `PipelineState` + `StepInfo` progress, WebSocket shell + Python REPL, model tier archiving on retrain
- `game_detail_cache.py` — per-game detail + SHAP result cache
- `response_cache.py` — short-TTL HTTP response memoization for hot read paths
- `timing.py` — `TimingMiddleware` + `timed_operation` context manager for structured latency logging
...
- Acts as a gRPC gateway: HTTP endpoints proxy to gRPC when stubs are available (unless `MLB_PREDICT_GRPC_ENABLED=0`).
- All HTML pages rendered via Jinja2 templates.
- Static assets served from `src/mlb_predict/app/static/`.
- **Deferred readiness**: when data and models already exist, `try_startup()` runs in a **background thread** so the server binds immediately; until `is_ready()` is true, `/` serves **`initializing.html`** (avoids browser connection errors during long loads).
- **MCP resilience**: if `create_mcp_app()` fails, the `/mcp` mount is skipped; MCP lifespan is nested inside the app lifespan when present.
- **Auto-bootstrap**: when data or models are missing, background tasks run `_auto_bootstrap()`, `_quick_train_bootstrap()`, or `_data_ingest_only()` per the **4-way startup matrix** (see README — data × models decision table; implemented in `main.py` lifespan).

## 13.2 In-Memory Cache

Location: `src/mlb_predict/app/data_cache.py`

Thread-safe. Holds all feature rows and the active production model. For v4 models, runs Stage 1 inference at startup. Per-game SHAP and live odds TTL caching are implemented in dedicated modules (`game_detail_cache`, odds path in main); HTTP responses use `response_cache.py`.

## 13.3 Admin Pipelines

Location: `src/mlb_predict/app/admin.py`

- `POST /api/admin/ingest` — full re-ingest (async, streaming progress). Optional JSON body: `include_preseason`, `seasons`, `refresh_mlbapi`, `refresh_retro`.
- `POST /api/admin/update` — update current season only. Same optional body shape as ingest.
- `POST /api/admin/retrain` — archive models for the target tier, then retrain. Optional JSON body: `training_tier` (`"quick"` \| `"full"`), `skip_cv`, `skip_stage1`.
- `GET` / `POST /api/admin/odds-config` — read / persist The Odds API key (`POST` body: `api_key`; key is not logged or re-exposed).
- `GET` / `POST /api/admin/betting-config` — Kelly %, bankroll budget, flat bet amount.
- `GET /api/admin/status` — data coverage, model inventory (per tier), pipeline states with **step lists** and elapsed times.
...
## 13.5 Sabermetrics dashboard (traditional + advanced)

The platform evolves from a **win-probability / odds model** into a combined **prediction + sabermetrics research dashboard**. Model features already ingest FanGraphs advanced team metrics and Statcast player signals; the dashboard exposes those statistics for human exploration independent of game-level predictions.

### 13.5.1 Product surfaces

| Surface | Path / API | Stat tier | Data source |
|---|---|---|---|
| Team standings (predicted vs actual) | `/standings` · `GET /api/standings` | Win–loss, run differential | Model + MLB Stats API |
| **Traditional team stats** | `/standings` (Batting / Pitching tabs) · `GET /api/team-stats` | AVG, OBP, SLG, OPS, R, HR, RBI, SB; ERA, WHIP, IP, SO | MLB Stats API (live) |
| **Advanced team stats** | `/standings` (Advanced tab) · `GET /api/team-advanced-stats` | wOBA, xwOBA, ISO, BABIP, barrel%, hard%; FIP, xFIP, K%, BB%, HR/FB | FanGraphs Parquet (`data/processed/fangraphs/`) |
| League leaders | `/leaders` · `GET /api/leaders` | Category tops (HR, ERA, …) | MLB Stats API |
| Full player tables | `/players` · `GET /api/player-stats` | Season counting + rate stats | MLB Stats API |
| **Advanced player stats** *(planned UI)* | `/stats/players` · `GET /api/player-advanced-stats` *(planned)* | xwOBA, barrel%, chase%, whiff%; SP/RP FIP, Stuff+ proxies | FanGraphs + Statcast Parquet (`data/processed/player/`) |
| Game-level feature attribution | `/game/{game_pk}` · SHAP panel | All 136 model inputs incl. FanGraphs diffs | Scored feature matrix |

### 13.5.2 Traditional vs advanced definitions

**Traditional (counting + classic rate stats)** — what appears on a baseball card or MLB.com team page:

- **Batting:** G, PA, AB, R, H, 2B, 3B, HR, RBI, BB, SO, SB, AVG, OBP, SLG, OPS
- **Pitching:** W, L, SV, IP, H, R, ER, BB, SO, HR, ERA, WHIP

**Advanced (sabermetric)** — already used inside the 136-feature model (§8.1.8–8.1.9):

- **Batting:** wOBA, xwOBA, ISO, BABIP, barrel%, hard-hit%, wRC+ *(team)*
- **Pitching:** FIP, xFIP, K%, BB%, HR/FB, SIERA *(where available)*
- **Player / lineup:** lineup-weighted xwOBA, barrel%, SP estimated xwOBA allowed

### 13.5.3 UI requirements (implementation backlog)

1. **Season sync:** Every stats view defaults to `current_mlb_season()` (§1.4) on cold and hot start.
2. **Tab layout on `/standings`:** Keep existing Standings / Team Batting / Team Pitching tabs for traditional stats; add **Advanced** tab with FanGraphs columns sortable via `makeSortable`.
3. **Dedicated `/stats` hub** *(optional)* linking team advanced, player advanced, and league percentile charts.
4. **Percentile badges:** For each advanced metric, show league percentile (0–100) using season-wide distribution from ingested Parquet.
5. **Offline resilience:** When `MLB_PREDICT_LIVE_API=0`, traditional team stats may be unavailable; advanced stats from local Parquet should still render for ingested seasons.
6. **Export:** Advanced tables participate in the global PDF/image export (`base.js`).

### 13.5.4 Planned API additions

| Method | Path | Description |
|---|---|---|
| `GET` | `/api/current-season` | MLB-aware default season for clients |
| `GET` | `/api/team-advanced-stats` | FanGraphs team batting + pitching advanced columns by season |
| `GET` | `/api/player-advanced-stats` | Paginated player advanced stats (Statcast + FanGraphs join) |

## 13.6 Hot-Swapping Models
...
| `GET` | `/api/current-season` | MLB-aware default season (§1.4) |
...
| `GET` | `/api/team-stats` | `season` | Team batting/pitching stats (traditional) |
| `GET` | `/api/team-advanced-stats` | `season` | FanGraphs advanced team metrics (Parquet) |
| `GET` | `/api/leaders` | `season`, `league_id`, `limit`, `stat_group` | League leaders |
| `GET` | `/api/player-stats` | `season`, `group`, `league_id`, `limit`, `offset` | Full player stats (traditional) |
| `GET` | `/api/player-advanced-stats` *(planned)* | `season`, `group`, `limit`, `offset` | Player advanced / Statcast stats |
...
| Method | Path | Description |
|---|---|---|
| `GET` | `/api/admin/status` | Full system status (pipelines include per-step progress) |
| `GET` | `/api/admin/odds-config` | Whether an Odds API key is configured (not the secret) |
| `POST` | `/api/admin/odds-config` | Persist Odds API key (`{"api_key": "..."}`) |
| `GET` | `/api/admin/betting-config` | Kelly / budget / flat bet settings |
| `POST` | `/api/admin/betting-config` | Update betting params (`kelly_pct`, `budget`, `bet_amount`) |
| `POST` | `/api/admin/ingest` | Full re-ingest (async); optional pipeline JSON body |
| `POST` | `/api/admin/update` | Update current season (async); optional pipeline JSON body |
| `POST` | `/api/admin/retrain` | Archive + retrain (async); optional `training_tier`, `skip_cv`, `skip_stage1` |
| `GET` | `/api/bootstrap-progress` | Aggregated bootstrap progress (ingest + retrain phases for initializing UI) |
...
| MLB-aware current season default (UI + API) | done |
| Sabermetrics dashboard — traditional team stats UI | done (standings batting/pitching tabs) |
| Sabermetrics dashboard — advanced team stats UI | done (standings Advanced tab + `/api/team-advanced-stats`) |
| Sabermetrics dashboard — advanced player stats UI | planned |
