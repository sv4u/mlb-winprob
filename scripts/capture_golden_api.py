...
def capture() -> None:
    ...
    captures.append(("api_seasons", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    # 4. teams
    r = client.get("/api/teams")
    captures.append(("api_teams", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    # 5. games
    r = client.get("/api/games", params={"limit": 5, "offset": 0})
    captures.append(("api_games", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    ...
    if game_pk is not None:
        ...
        captures.append((f"api_game_detail_{game_pk}", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    else:
        captures.append(("api_game_detail_placeholder", {"_skip": "no game_pk from /api/games"}))

    # 7. upsets
    r = client.get("/api/upsets", params={"limit": 5})
    captures.append(("api_upsets", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    # 8. cv-summary
    captures.append(("api_cv_summary", get("/api/cv-summary")))
    # 9. standings
    r = client.get("/api/standings", params={"season": 2026})
    captures.append(("api_standings", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    # 10. team-stats
    r = client.get("/api/team-stats", params={"season": 2026})
    captures.append(("api_team_stats", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
    # 11. active-model
    ...
    captures.append(("api_switch_model", r.json() if r.status_code == 200 else {"_status": r.status_code, "body": r.json()}))
